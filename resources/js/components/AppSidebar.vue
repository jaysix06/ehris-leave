...
const NAV_ACCESS_BY_ROLE: Record<string, RoleAccessRule> = {
    hidden: {
        ...
        hidden: [
            ...
            'self-service.deped-email-requests',
            'request-status.my-leave',
            'survey',
            'employee-management.employee-profile',
            'employee-management.psipop-update',
            'employee-management.deped-email-requests',
    ...
    reporting_manager: {
        ...
        hidden: ['employee-management.employee-tasks'],
    },
    sds: {
        allow: [
            'dashboard',
            ...
            'reports',
            'survey',
            'utilities.employee-list',
            'utilities.job-title-monthly-salary',
            'utilities.reporting-manager',
            'utilities.survey-management',
            'utilities.pop-up-management',
            'utilities.announcement-management',
        ],
        hidden: ['employee-management.employee-tasks'],
    },
    employee: {
...
const currentUser = computed<Record<string, unknown> | undefined>(
    () => page.props.auth?.user as Record<string, unknown> | undefined,
);
// Step 1: resolve current user role into one of our sidebar role keys.
const currentRoleKey = computed<string>(() =>
    resolveRoleKey(currentUser.value?.role),
);
const currentRoleAccess = computed<RoleAccessRule>(
    () =>
        NAV_ACCESS_BY_ROLE[currentRoleKey.value] ?? NAV_ACCESS_BY_ROLE.employee,
);
...
const parsedPermissionAccess = computed<{
    allow: Set<string>;
    hidden: Set<string>;
}>(() => {
...
const canViewNavKey = (key: string): boolean => {
    ...
    return (
        hasRoleWildcard ||
        hasPermissionWildcard ||
        roleAllowed.includes(normalizedKey) ||
        parsedPermissionAccess.value.allow.has(normalizedKey)
    );
};

const buildVisibleNavItems = (
    ...
    for (const item of items) {
        ...
        const visibleChildren = item.children
            ? buildVisibleNavItems(item.children, hiddenKeys, isItemAllowed)
            : [];
...
const surveyCategoriesWithSurveys = computed(() => {
    const cats =
        (page.props.surveyCategoriesWithSurveys as string[] | undefined) ?? [];
    return Array.isArray(cats) ? cats : [];
});

const mainNavItems = computed<NavItem[]>(() => {
    const surveyChildren: SidebarNavItem[] =
        surveyCategoriesWithSurveys.value.map((category) => ({
            key: 'survey',
            title: category,
    ...
    const items: SidebarNavItem[] = [
        ...
        },
        {
            ...
            children: [
                ...
                {
                    key: 'employee-management.employee-tasks',
        ...
        },
        {
            ...
            children: [
                ...
                {
                    key: 'self-service.locator-slip',
                    title: 'Locator Slip',
                    href: selfServiceRoutes.locatorSlip(),
                },
            ],
        },
        {
        ...
        },
        {
            ...
            children: [
                {
                    key: 'my-details.official-info',
                    title: 'Official Info',
                    href: myDetails({ query: { section: 'official-info' } }),
                },
                {
                    key: 'my-details.personal-info',
                    title: 'Personal Info',
                    href: myDetails({ query: { section: 'personal-info' } }),
                },
                {
                    key: 'my-details.family-background',
                    title: 'Family Background',
                    href: myDetails({
                        query: { section: 'family-background' },
                    }),
                },
                {
                    key: 'my-details.education-background',
                    title: 'Education Background',
                    href: myDetails({
                        query: { section: 'education-background' },
                    }),
                },
                {
                    key: 'my-details.eligibility',
                    title: 'Eligibility',
                    href: myDetails({ query: { section: 'eligibility' } }),
                },
                {
                    key: 'my-details.work-experience',
                    title: 'Work Experience',
                    href: myDetails({ query: { section: 'work-experience' } }),
                },
                {
                    key: 'my-details.voluntary-work',
                    title: 'Voluntary Work',
                    href: myDetails({ query: { section: 'voluntary-work' } }),
                },
                {
                    key: 'my-details.training',
                    title: 'Training',
                    href: myDetails({ query: { section: 'training' } }),
                },
                {
                    key: 'my-details.others',
                    title: 'Others',
                    href: myDetails({ query: { section: 'others' } }),
                },
            ],
        },
        {
