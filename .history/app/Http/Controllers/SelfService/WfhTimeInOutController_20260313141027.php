...
class WfhTimeInOutController extends Controller
{
    ...
    private function buildWfhPdfHtml(
        ...
        $hWithBreaks = fn (string $s) => nl2br($h($s));
        ...
        foreach ($tasks as $task) {
            ...
            $targetChunks = $this->splitWfhPdfCellIntoRowChunks($targetedTask, 34, $maxLinesPerChunk);
            $accomplishmentChunks = $this->splitWfhPdfCellIntoRowChunks($accomplishment, 30, $maxLinesPerChunk);
            $chunkCount = max(count($targetChunks), count($accomplishmentChunks));

            for ($chunkIndex = 0; $chunkIndex < $chunkCount; $chunkIndex++) {
                $targetChunk = (string) ($targetChunks[$chunkIndex] ?? '');
                $accomplishmentChunk = (string) ($accomplishmentChunks[$chunkIndex] ?? '');
                $dateRangeChunk = $chunkIndex === 0 ? $dateRange : '';
                $priorityChunk = $chunkIndex === 0 ? $priority : '';
                $stationChunk = $chunkIndex === 0 ? $station : '';

                $rowUnits = $this->estimateWfhPdfRowUnits(
                    $targetChunk,
                    $accomplishmentChunk,
                    $dateRangeChunk,
                    $priorityChunk,
                ...
                $currentPageRows .= '<tr>';
                $currentPageRows .= '<td class="col-task">'.$hWithBreaks($targetChunk).'</td>';
                $currentPageRows .= '<td class="col-accomplishment">'.$hWithBreaks($accomplishmentChunk).'</td>';
                $currentPageRows .= '<td class="col-date">'.$hWithBreaks($dateRangeChunk).'</td>';
                $currentPageRows .= '<td class="col-priority">'.$hWithBreaks($priorityChunk).'</td>';
                $currentPageRows .= '<td class="col-station">'.$hWithBreaks($stationChunk).'</td>';
                $currentPageRows .= '</tr>';
...
th.col-date, td.col-date { font-family: "BookmanOldStyle" !important; }
.table-block {
    width: 100%;
}
...
HTML;
    ...
    private function estimateWfhPdfRowUnits(
        string $targetedTask,
        ...
        $dateLines = $this->estimateWfhPdfWrappedLines($dateRange, 16);
        $priorityLines = $this->estimateWfhPdfWrappedLines($priority, 8);
        $stationLines = $this->estimateWfhPdfWrappedLines($station, 18);

        return (float) max($targetLines, $accomplishmentLines, $dateLines, $priorityLines, $stationLines) + 0.45;
    }

    /**
     * @return array<int, string>
     */
    private function splitWfhPdfCellIntoRowChunks(string $value, int $maxCharsPerLine, int $maxLinesPerChunk): array
    {
        ...
        foreach ($segments as $segment) {
            $wrappedSegment = wordwrap($segment, max(1, $maxCharsPerLine), "\n", true);
            $segmentLines = preg_split('/\n/u', $wrappedSegment) ?: [''];

            foreach ($segmentLines as $segmentLine) {
                $wrappedLines[] = $segmentLine;
            }
        }
        ...
        foreach ($wrappedLines as $wrappedLine) {
            $currentChunk[] = $wrappedLine;

            if (count($currentChunk) >= max(1, $maxLinesPerChunk)) {
                $chunks[] = implode("\n", $currentChunk);
                $currentChunk = [];
            }
        }
    ...
    private function resolveWfhPdfNameFontPath(): ?string
    {
        ...
        foreach ($fallbackCandidates as $fallbackPath) {
            if (is_file($fallbackPath) && is_readable($fallbackPath)) {
                return (string) (realpath($fallbackPath) ?: $fallbackPath);
            }
        }
