...
class MyDetailsController extends Controller
{
    ...
    public function exportPdsExcel(Request $request): StreamedResponse
    {
        ...
        // Table rows 51–55 so they do not overlap children (rows 37–48). Adjust if your template uses different rows.
        // Columns: B = LEVEL, C = Name of school, D = Degree/course, E = From, F = To, G = Highest level/units, H = Year graduated, I = Scholarship.
        $educationLevels = [
            53 => 'ELEMENTARY',
            54 => 'SECONDARY',
            55 => 'VOCATIONAL / TRADE COURSE',
            56 => 'COLLEGE',
            57 => 'GRADUATE STUDIES',
        ];
        foreach ($educationLevels as $rowNum => $levelLabel) {
            $record = $this->findEducationRecordByLevel($education, $levelLabel);
            $cellMap["B{$rowNum}"] = $this->pdsValue($levelLabel);
            $cellMap["C{$rowNum}"] = $this->pdsValue($record->school_name ?? null);
            $cellMap["D{$rowNum}"] = $this->pdsValue($record->course ?? null);
            $cellMap["E{$rowNum}"] = $this->pdsValue($record->from_year ?? null);
            $cellMap["F{$rowNum}"] = $this->pdsValue($record->to_year ?? null);
            $cellMap["G{$rowNum}"] = $this->pdsValue($record->highest_grade ?? null);
            $cellMap["H{$rowNum}"] = $this->pdsValue($record->year_graduated ?? null);
            $cellMap["I{$rowNum}"] = $this->pdsValue($record->scholarship ?? null);
        }
