...
class MyDetailsController extends Controller
{
    ...
    public function exportPdsExcel(Request $request): StreamedResponse
    {
        ...
        $cellMap = [
            ...
            'I31' => $contactInfo->zip_code1 ?? null,                                // Permanent address ZIP Code
        ...
        // III. EDUCATIONAL BACKGROUND (Annex H C1 / CS Form 212).
        // Per PDS: "FOR TWO OR MORE COURSES, JUST ADD ROWS AND MERGE THE LEVEL". Use N/A when no data.
        // Order: ELEMENTARY, SECONDARY, VOCATIONAL, all COLLEGE rows, GRADUATE STUDIES. Table rows 54, 55, 56, …
        // Template table must include at least 6 rows (54–59) when employee has multiple college degrees so both degrees and GRADUATE STUDIES show.
        // Columns: B=LEVEL, D=Name of school, G=Degree/course, J=From, K=To, L=Highest level/units, M=Year graduated, N=Scholarship.
        $educationRows = $this->buildEducationExportRows($education);
        $educationStartRow = 54;
        foreach ($educationRows as $index => $row) {
            $rowNum = $educationStartRow + $index;
            $cellMap["B{$rowNum}"] = $this->pdsValue($row['level_label']);
            $cellMap["D{$rowNum}"] = $this->pdsValue($row['school_name'] ?? null);
            $cellMap["G{$rowNum}"] = $this->pdsValue($row['course'] ?? null);
            $cellMap["J{$rowNum}"] = $this->pdsValue($row['from_year'] ?? null);
            $cellMap["K{$rowNum}"] = $this->pdsValue($row['to_year'] ?? null);
            $cellMap["L{$rowNum}"] = $this->pdsValue($row['highest_grade'] ?? null);
            $cellMap["M{$rowNum}"] = $this->pdsValue($row['year_graduated'] ?? null);
            $cellMap["N{$rowNum}"] = $this->pdsValue($row['scholarship'] ?? null);
        }
    ...
    /**
     * Find an education record that matches the given PDS level label.
     * Returns a stdClass with school_name, course, from_year, to_year, highest_grade, year_graduated, scholarship
     * (all null if no match).
     *
    ...
    private function findEducationRecordByLevel($education, string $levelLabel): object
    {
        ...
        foreach ($education as $row) {
            ...
            $matches = match (true) {
                ...
                // GRADUATE STUDIES first: Masteral, MAED, Graduate Studies (not "College GRADUATE" = graduated from college).
    ...
    /**
     * Return all education records that match the given PDS level (for "two or more courses" – multiple rows per level).
     *
     * @param  \Illuminate\Support\Collection<int, object>  $education
    ...
    /**
     * Build ordered list of education rows for PDS export.
     * Per CS Form 212: one row per level minimum (N/A if no data); multiple rows per level when "two or more courses".
     * GRADUATE STUDIES is always last so it stays in the template table (e.g. row 59 when table is 54–59).
     * School name and degree/course are written in full (do not abbreviate).
     *
     * @param  \Illuminate\Support\Collection<int, object>  $education
    ...
    private function setWorksheetCells(string $worksheetXml, array $cellMap): string
    {
        ...
        foreach ($cellMap as $cellRef => $value) {
            ...
            if (! $rowNode instanceof \DOMElement) {
                ...
                $prevRow = $rowNumber > 1 ? $xpath->query("/s:worksheet/s:sheetData/s:row[@r='".($rowNumber - 1)."']")->item(0) : null;
                if ($prevRow instanceof \DOMElement) {
                    foreach ($prevRow->attributes as $attr) {
                        if ($attr->name !== 'r' && $attr->namespaceURI !== null) {
                            $rowNode->setAttributeNS($attr->namespaceURI, $attr->name, $attr->value);
                        } elseif ($attr->name !== 'r') {
