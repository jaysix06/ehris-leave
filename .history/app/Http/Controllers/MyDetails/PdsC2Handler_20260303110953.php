...
class PdsC2Handler
{
    ...
    public function buildCellMap($eligibility, $workExperience, callable $formatDate, callable $pdsValue): array
    {
        ...
        foreach ($workExperience->take(self::WORK_EXPERIENCE_MAX_ROWS)->values() as $i => $row) {
            ...
            $map["A{$r}"] = ($from === 'N/A' && $to === 'N/A') ? 'N/A' : trim($from.' - '.$to);
            ...
            $step = $row->step ?? null;
            if ($salaryGrade !== null || $step !== null) {
                $salaryGradeStep = trim((string) $salaryGrade.(($salaryGrade !== null && $step !== null) ? ' / ' : '').(string) $step);
            }
            $map["F{$r}"] = $pdsValue($salaryGradeStep);
