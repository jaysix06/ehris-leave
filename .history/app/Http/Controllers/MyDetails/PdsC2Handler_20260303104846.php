...
class PdsC2Handler
{
    ...
    public function buildCellMap($eligibility, $workExperience, callable $formatDate, callable $pdsValue): array
    {
        ...
        foreach ($eligibility->take(self::ELIGIBILITY_MAX_ROWS)->values() as $i => $row) {
            ...
            $map["B{$r}"] = $pdsValue($row->title ?? null);
            $map["C{$r}"] = $pdsValue($row->rating ?? null);
            $map["D{$r}"] = $pdsValue($formatDate($row->date_exam ?? null));
            $map["E{$r}"] = $pdsValue($row->place_exam ?? null);
            $map["F{$r}"] = $pdsValue($row->license_no ?? null);
        }
        ...
        foreach ($workExperience->take(self::WORK_EXPERIENCE_MAX_ROWS)->values() as $i => $row) {
            ...
            $map["C{$r}"] = $pdsValue($row->position_title ?? null);
            $map["D{$r}"] = $pdsValue($row->company_name ?? null);
            $map["E{$r}"] = $pdsValue($row->monthly_salary ?? null);
            $map["F{$r}"] = $pdsValue($row->salary_grade_step ?? null);
            $map["G{$r}"] = $pdsValue($row->employment_status ?? null);
            $map["H{$r}"] = $pdsValue($row->government_service ?? null);
        }
