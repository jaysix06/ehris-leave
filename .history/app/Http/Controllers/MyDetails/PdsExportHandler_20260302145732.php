...
class PdsExportHandler
{
    ...
    /** Education table columns we write (A, B, C for level/gray; D, G, J, K, L, M, N for data per PDS). */
    private const EDUCATION_TABLE_COLUMNS = ['A', 'B', 'C', 'D', 'G', 'J', 'K', 'L', 'M', 'N'];
    ...
    private function getEducationStyleMapFromTemplate(string $worksheetXml): ?array
    {
        ...
        $row58 = $getRowStyles(58);
        if ($row54 === null || $row58 === null) {
            return null;
        }

        $map = [];
        foreach (self::EDUCATION_TABLE_COLUMNS as $col) {
            if (isset($row54[$col], $row58[$col])) {
                $map[$col] = [$row54[$col], $row58[$col]];
            }
        }
        ...
        foreach ($cellMap as $cellRef => $value) {
            ...
            if ($rowNumber >= $educationStartRow && $rowNumber <= $educationEndRow) {
                ...
                if ($educationStyleMap !== null && preg_match('/^([A-Z]+)\d+$/', $cellRef, $m)) {
                    $col = $m[1];
                    if (isset($educationStyleMap[$col])) {
                        $isLastRow = $rowNumber === $educationEndRow;
                        $styleToApply = $educationStyleMap[$col][$isLastRow ? 1 : 0];
                    }
                }
